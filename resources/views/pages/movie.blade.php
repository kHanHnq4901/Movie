                           ...
                           @else
                              @php
                                 ...
                                 foreach($movie->episode as $episode) {
                                    ...
                                    if (isset($episode->linkVip) && $episode->linkVip->status == 1) {
                                       $linkVip = $linkVip + 1;
                                    } else if (isset($episode->linkVip) && $episode->linkVip->status == 0) {
                                       $linkDefault = $linkDefault + 1;
                                    }
                                 }
                              @endphp

                              @if (Session::has('publisher_id'))
                                 @if ($is_vip == 0 && $linkDefault == 0)
                                    <a href="#!" class="bwac-btn" onclick="showUpdateVipAlert()">
                                       <i class="fa fa-play"></i>
                                    </a>
                                 @elseif($is_vip == 1 && $linkVip > 0)
                                    @foreach($movie->episode as $episode)
                                       @if (isset($episode->linkVip) && $episode->linkVip->status == 1)
                                          <a href="{{ url('xem-phim/'.$movie->slug.'/tap-'.$episode->episode.'/server-'.$episode->link) }}" class="bwac-btn">
                                             <i class="fa fa-play"></i>
                                          </a>
                                          @break
                                       @endif
                                 ...
                                 @else
                                    @foreach($movie->episode as $episode)
                                       @if (isset($episode->linkVip) && $episode->linkVip->status == 0)
                                          <a href="{{ url('xem-phim/'.$movie->slug.'/tap-'.$episode->episode.'/server-'.$episode->link) }}" class="bwac-btn">
                                             <i class="fa fa-play"></i>
                                          </a>
                                          @break
