...
class IndexController extends Controller
{
    ...
    public function login_publisher(Request $request){
        ...
        if ($publisher) {
            ...
            Session::put('start_vip',$publisher->start_vip);
            Session::put('end_vip', $publisher->end_vip);
            $is_vip = 0;

            $currentTime = Carbon::now();
            if (!empty($publisher->end_vip) && $publisher->end_vip >= $currentTime) {
                $is_vip = 1;
            }

            Session::put('is_vip', $is_vip);
    ...
    public function bookmark(Request $request){
        ...
        if ($check) {
            return response([
                'code' => 404,
                'message' => 'Phim đã có trong tủ phim.'
            ]);
        } else {
            ...
            $bookmarks = Bookmark::with('movie')->where('publisher_id', $data['publisher_id'])->get();

            $html = view('pages.include.bookmark', compact('bookmarks'))->render();

            return response([
                'code' => 200,
                'message' => 'Thêm thành công phim vào tủ phim.',
                'html' => $html,
                'number_movie' => $bookmarks->count()
            ]);
        }
    }
    ...
    public function delete_bookmark($bookmark_id)
        {
            try {
                ...
                if ($bookmark->delete()) {
                    ...
                    $bookmarks = Bookmark::with('movie')->where('publisher_id', $publisher_id)->get();

                    $html = view('pages.include.bookmark', compact('bookmarks'))->render();

                    return response([
                        'code' => 200,
                        'message' => 'Xóa thành công phim khỏi tủ phim.',
                        'html' => $html,
                        'number_movie' => $bookmarks->count()
                    ]);
                } else {
                    return response([
                        'code' => 404,
                        'message' => 'Đã sảy ra lỗi không thể xóa phim khỏi tủ phim'
                    ]);
                }
    ...
    public function delete_all_bookmark() {
        ...
        try {
            ...
            return response([
                'code' => 200,
                'message' => 'Xóa thành công tất cả phim khỏi tủ phim.',
            ]);
        } catch (\Exception $e) {
            return response([
                'code' => 404,
    ...
    public function movie($slug){
        ...
        $movie= Movie::with(['category','genre','country','movie_genre','episode' => function ($query) {
            $query->with(['linkVip' => function ($link) {
                $link->orderBy('status');
            }]);
    ...
    public function watch($slug,$tap,$server_active){
        ...
        $episode_list = Episode::with(['linkVip'])->where('movie_id',$movie->id)->orderBy('episode','ASC')->get();
        //dd($episode_list->toArray());
